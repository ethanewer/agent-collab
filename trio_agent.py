"""Trio agent: three Claude Code instances collaborating via a shared append-only log file."""
...
AGENT_C_LOG = "/tmp/agent-c.log"

COLLAB_PROMPT_A = textwrap.dedent("""\
    You are one of three equal AI agents (you are Agent A; the others are Agent B \
    and Agent C) working together in the same environment on the same task.
...
COLLAB_PROMPT_B = textwrap.dedent("""\
    You are one of three equal AI agents (you are Agent B; the others are Agent A \
    and Agent C) working together in the same environment on the same task.
    ...
    Only take over another agent's claimed work if:
...
COLLAB_PROMPT_C = textwrap.dedent("""\
    You are one of three equal AI agents (you are Agent C; the others are Agent A \
    and Agent B) working together in the same environment on the same task.

    COMMUNICATION: You share a log file at {collab_file}.
    - Append only: echo "[C|$(date -u +%Y-%m-%dT%H:%M:%S)] <msg>" >> {collab_file}
    - Read it with: cat {collab_file}
...
class TrioClaudeCode(BaseInstalledAgent):

    @staticmethod
    def name() -> str:
        return "claude-code-trio"
    ...
    async def run(
        ...
        sessions_c = f"{agent_dir}/sessions-c"
        ...
        saved_c_log = f"{agent_dir}/agent-c-output.log"

        escaped_instruction = shlex.quote(instruction)
        escaped_prompt_a = shlex.quote(COLLAB_PROMPT_A)
        escaped_prompt_b = shlex.quote(COLLAB_PROMPT_B)
        escaped_prompt_c = shlex.quote(COLLAB_PROMPT_C)

        subdirs = "debug projects/-app shell-snapshots statsig todos skills"
        setup_command = (
            f"mkdir -p {sessions_a}/{{{subdirs}}} "
            f"{sessions_b}/{{{subdirs}}} "
            f"{sessions_c}/{{{subdirs}}} && "
            f"touch {COLLAB_FILE}"
        )
        ...
        run_command = (
            ...
            f"CLAUDE_CONFIG_DIR={sessions_c} "
            f"claude --verbose --output-format=stream-json "
            f"--permission-mode=bypassPermissions "
            f"--append-system-prompt {escaped_prompt_c} "
            f"--print -- {escaped_instruction} "
            f">{AGENT_C_LOG} 2>&1 & PID_C=$!; "
            ...
            f"cp {AGENT_C_LOG} {saved_c_log} 2>/dev/null || true; "

            'echo "Agent A exit=$EXIT_A, Agent B exit=$EXIT_B, Agent C exit=$EXIT_C"; '
            "[ $EXIT_A -eq 0 ] || [ $EXIT_B -eq 0 ] || [ $EXIT_C -eq 0 ]"
        )
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        ...
        total_input = 0
        total_output = 0
        for suffix in ("sessions-a", "sessions-b", "sessions-c"):
