"""Duo agent: one Opus 4.6 + one Sonnet 4.6 Claude Code, collaborating via a shared log.

Random assignment (seed=42): agent p = opus, agent q = sonnet.
"""
...
MODEL_P = "claude-opus-4-6"
MODEL_Q = "claude-sonnet-4-6"
...
class DuoMixedClaudeCode(BaseInstalledAgent):
    @staticmethod
    def name() -> str:
        return "claude-code-duo-mixed"
    ...
    def _build_env(self, model: str) -> dict[str, str]:
        env: dict[str, str] = {}
    ...
    async def run(
        ...
        # Agent p gets opus, agent q gets sonnet
        env_p = self._build_env(MODEL_P)
        env_q = self._build_env(MODEL_Q)

        # Merge both envs for the setup phase (shared API key, etc.)
        env_setup = self._build_env(MODEL_P)
        ...
        escaped_prompt_p = shlex.quote(prompt_p)
        escaped_prompt_q = shlex.quote(prompt_q)
        ...
        await self.exec_as_agent(environment, command=setup_command, env=env_setup)

        # Build env strings for inline export in the shell command
        def env_exports(env_dict: dict[str, str]) -> str:
            parts = []
            for k, v in env_dict.items():
                parts.append(f"export {k}={shlex.quote(v)}")
            return "; ".join(parts)

        exports_p = env_exports(env_p)
        exports_q = env_exports(env_q)
        ...
        run_command = (
            ...
            f"--append-system-prompt {escaped_prompt_p} "
            ...
            f"claude --verbose --output-format=stream-json "
            f"--permission-mode=bypassPermissions "
            f"--append-system-prompt {escaped_prompt_q} "
            f"--print -- {escaped_instruction} "
            ...
            'echo "Agent P (opus) exit=$EXIT_A, Agent Q (sonnet) exit=$EXIT_B"; '
            "[ $EXIT_A -eq 0 ] || [ $EXIT_B -eq 0 ]"
        )

        try:
            await self.exec_as_agent(environment, command=run_command, env=env_setup)
        except Exception:
            save_cmd = (
                f"cp {COLLAB_FILE} {saved_collab} 2>/dev/null || true; "
                f"cp {AGENT_A_LOG} {saved_a_log} 2>/dev/null || true; "
